...
1. **Gleichgewicht**, **Stabilität** und **Einzugsgebiet**: stabil heisst, das System kehrt nach einer kleinen Störung zurück,
...
Man vergleicht beide $\alpha$ mit 1. Davon hängt ab, ob am Ende eine Art, beide Arten oder — je nach Start — die eine oder die andere übrig bleibt.
...
Otter fressen Seeigel, Seeigel fressen Kelp. Die Kette läuft in dieselbe Richtung wie Wolf, Elch und Ufervegetation, hier aus dem Freiland.
...
Langendorf et al. (2025), aus Zeitreihen von etwa 30 Jahren. Westküste von Vancouver Island und San Nicolas Island.

![](../../pics/keystone/langendorf_2025_fig1.jpg){fig-alt="Geschaetztes Kelp-Food-Web mit Ottern, Raeubern, Herbivoren und Algen an zwei Kueste" height="300px"}
...
Die Kette Otter–Igel–Kelp ist darin eine Kante unter vielen.

Violett gibt es nur im Norden, grün nur im Süden. Durchgezogen wirkt nach unten, gestrichelt nach oben.

## Die erwartete Kaskade

An der kalifornischen Küste regulieren Seeotter Seeigel. Ohne Otter entsteht oft eine Seeigelwüste.

![](../../pics/keystone/guardians_of_the_kelp.png){fig-alt="Mit Seeottern bleibt der Kelpwald; ohne nehmen Seeigel zu und fressen Kelp weg" height="300px"}
...
![](../../pics/keystone/figure10_otter_urchin_kelp.jpg){fig-alt="Zeitreihen Otter, Kelp, Seeigel, Pycnopodia und Mechanismus-Skizze" height="340px"}

::: aside
Smith et al. (2021), Fig. 10 · Jessica Kendall-Bar · Planet-Vie/PNAS · Lehre / nicht-kommerziell
:::

Otter oben (A), Kelp unten (B), Seeigel oben (C), *Pycnopodia* weg (D). Die Otterdichte steigt wieder, der Kelpwald aber nicht überall.
...
## Restwald und Seeigelwüste

Start im Kelpwald. Die Otterdichte sinkt, bis der Wald in die Seeigelwüste kippt. Steigt sie wieder, bleibt die Wüste, bis eine höhere Dichte erreicht ist. Auf der Skizze liegen die beiden Sprünge nicht übereinander.

Otter können vorhandenen Kelp schützen. Eine Seeigelwüste mit derselben Dichte wieder in Wald zu verwandeln gelingt oft nicht.

Einen bestehenden Wald zu halten und eine Seeigelwüste wiederherzustellen sind zwei verschiedene Aufgaben.

::: aside
Dieselbe Kurvenform kommt aus einem Insektenmodell. Bonus am Ende der Folien: der Fichtenwickler.
:::

## Wie der Otter jagt

Der Unterschied ist **wie** der Otter wirkt — nicht wie viele Seeigel es gibt.

**Nicht:** je mehr Beute, desto mehr Frass.

**Sondern:** nur wenn die Beute gut genug ist, jagt er.

Ein Tauchgang kostet. In der Seeigelwüste sind die Igel mager — der Otter lässt sie, obwohl es viele gibt.

Im Kelpwald sind sie fett. Dort lohnt sich der Tauchgang.

## Holling Typ III {#holling-typ-iii}

Der Verlust der Seeigel $U$ durch Otter $P$ ist dann

$$
P\cdot f(U),\qquad f(U)=\frac{a U^{2}}{1+ah U^{2}}
$$
...
#| out-width: 56%
source("../slides-ggplot.R")
U <- seq(0, 12, length.out = 250)
a <- 0.08
h <- 0.5
holling <- data.frame(U = U, f = a * U^2 / (1 + a * h * U^2))
ggplot(holling, aes(U, f)) +
  geom_line(colour = col_navy, linewidth = 1.15) +
  geom_hline(yintercept = 1 / h, linetype = "dashed", colour = "#7f8c8d") +
  annotate("text", x = 2.1, y = 0.22, label = "kein Jagen",
           colour = "#7f8c8d", size = 3.2) +
  annotate("text", x = 10.2, y = 1 / h + 0.16, label = "Saettigung",
           colour = "#7f8c8d", size = 3.2) +
  labs(x = "Seeigel-Biomasse", y = "Frass pro Otter") +
  theme_slide()
```

Links bleibt $f$ fast null: kein Jagen. Rechts sättigt $f$ bei $1/h$: volle Mahlzeit. Dieselbe Form steht im Bonus.
...
*Pisaster* erhält die Vielfalt, weil er die starke Konkurrentin frisst. Otter halten den Wald, weil sie nur jagen, wo die Igel sich lohnen. Nach dem Zusammenbruch reicht dieselbe Otterdichte oft nicht: der Rückweg hat eine andere Schwelle.

Ob ein Zustand hält, sieht man daran, wie das System auf die Störung reagiert.
