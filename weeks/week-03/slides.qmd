...
Otter fressen Seeigel selektiv. Ein Tauchgang lohnt sich erst, wenn die Biomasse gross genug ist. Bei wenig Beute bleibt der Frass deshalb flach. Danach steigt er und sättigt:
...
#| fig-width: 7.2
#| out-width: 48%
N <- seq(0, 8, length.out = 200)
a <- 0.9
h <- 0.5
holling <- data.frame(N = N, f = a * N^2 / (1 + a * h * N^2))
ggplot(holling, aes(N, f)) +
  geom_line(colour = col_navy, linewidth = 1.15) +
  geom_hline(yintercept = 1 / h, linetype = "dashed", colour = "#7f8c8d") +
  annotate("text", x = 6.4, y = 1 / h + 0.12, label = "Saettigung",
           colour = "#7f8c8d", size = 3.3) +
  labs(x = "Seeigel-Biomasse", y = "Frass pro Otter") +
  theme_slide()
```

Bei kleiner Biomasse bleibt die Kurve flach, bei grosser nähert sie sich $1/h$. Dieselbe Form steht im Bonus, beim Fichtenwickler.
...
*Pisaster* erhält die Vielfalt, weil er die starke Konkurrentin frisst. Nach dem Zusammenbruch des Kelps führt eine höhere Otterdichte nicht von selbst zur Erholung: der Rückweg hat eine andere Schwelle.
