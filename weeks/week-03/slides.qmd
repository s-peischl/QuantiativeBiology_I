...
Kursstandard (Euler / Differenzen) — wie im Code:

\[
\begin{aligned}
N_{1,t+1}
  &= N_{1,t}
     + r_1 N_{1,t}\left(1 - \frac{N_{1,t} + \alpha_{12} N_{2,t}}{K_1}\right)\Delta t \\[0.35em]
N_{2,t+1}
  &= N_{2,t}
     + r_2 N_{2,t}\left(1 - \frac{N_{2,t} + \alpha_{21} N_{1,t}}{K_2}\right)\Delta t
\end{aligned}
\]
...
Literatur schreibt oft \(\mathrm{d}N_i/\mathrm{d}t=\ldots\); wir simulieren den **Euler-Schritt**.  
Parameter hängen von \(\Delta t\) ab (siehe Woche 2).
...
## Punchline: Start egal — Gewinner fest \[SIM\]
...
## Simulation: Start entscheidet \[SIM\]
...
## Beispiel Punkt: Exclusion-Endpunkt \[SIM\]
...
## Beispiel Bistabil: Gras vs. Wald \[SIM\]
...
## Beispiel Zyklus: Räuber–Beute \[SIM\]
...
Euler-Form (Kursstandard), \(X^+ = X_{t+1}\):

\[
\begin{aligned}
A^{+}
  &= A
     + \Bigl[r_A A\bigl(1 - \tfrac{A + \alpha_{AS} S}{K}\bigr)
            - a_{\mathrm{EA}} A E\Bigr]\Delta t \\[0.35em]
S^{+}
  &= S
     + \Bigl[r_S S\bigl(1 - \tfrac{S + \alpha_{SA} A}{K}\bigr)
            - a_{\mathrm{ES}} S E\Bigr]\Delta t \\[0.35em]
E^{+}
  &= E
     + \Bigl[e_E(a_{\mathrm{EA}} A + a_{\mathrm{ES}} S)E
            - m_E E - a_{\mathrm{WE}} E W\Bigr]\Delta t \\[0.35em]
W^{+}
  &= W
     + \bigl(e_W a_{\mathrm{WE}} E W - m_W W\bigr)\Delta t
\end{aligned}
\]
...
Literatur oft mit \(\dot A,\dot S,\ldots\); Parameter gelten für **unser** \(\Delta t\).

## Parameter (Unterrichts-Set)

| Symbol | Wert | Bedeutung |
|--------|------|-----------|
| \(r_A, r_S\) | 1.2, 1.0 | Wachstum Aspen / Sage |
| \(\alpha_{AS}, \alpha_{SA}\) | 0.55, 0.70 | Konkurrenz |
| \(a_{\mathrm{EA}}\) | 0.035 | starke Beweidung Aspen (\(a_{\mathrm{elk,aspen}}\)) |
| \(a_{\mathrm{ES}}\) | 0.020 | schwache Beweidung Sage (\(a_{\mathrm{elk,sage}}\)) |
| \(a_{\mathrm{WE}}\) | 0.040 | Wolf frisst Elk (\(a_{\mathrm{wolf,elk}}\)) |
| \(m_E, m_W\) | 0.10, 0.30 | Mortalität Elk / Wolf |

## Schritt A: nur Pflanzen \[SIM\]
...
matplot(plants$time, cbind(plants$N1, plants$N2), type = "l", lty = 1, lwd = 2.5,
        ...
        xlab = "Zeit", ylab = "Dichte", main = "Nur Aspen + Sage: Koexistenz [SIM]")
...
## Schritt B: + Elk, kein Wolf → Aspen Exclusion \[SIM\]
...
matplot(noW$time, noW[, names(cols)], type = "l", lty = 1, lwd = 2.4,
        col = cols, xlab = "Zeit", ylab = "Haeufigkeit",
...
## Schritt C: + Wolf → Aspen gerettet, Elk mittel \[SIM\]
...
## Erweitertes Modell \[SIM\] — ohne Wolf
...
matplot(noW_rich$time, noW_rich[, names(rich_cols)], type = "l", lty = 1,
        lwd = 2.2, col = rich_cols, xlab = "Zeit", ylab = "Haeufigkeit",
...
## Erweitertes Modell \[SIM\] — mit Wolf
