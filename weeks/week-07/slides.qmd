...
zeige_demen <- function(sim, main) {
  long <- rbind(
    data.frame(generation = sim$generation, p = sim$p1, pop = "See"),
    data.frame(generation = sim$generation, p = sim$p2, pop = "Meer")
  )
  ggplot(long, aes(generation, p, colour = pop)) +
    geom_line(linewidth = 1.05) +
    scale_colour_manual(values = c(See = "#2166ac", Meer = "#b2182b")) +
    coord_cartesian(ylim = c(0, 1)) +
    labs(x = "Generation", y = "Frequenz von A", title = main) +
    theme_slide()
}
```
...
#| fig-height: 4
#| fig-width: 9
scan$band <- factor(scan$chr %% 2)
treffer <- scan[scan$label != "", ]
ggplot(scan, aes(x, fst, colour = band)) +
  geom_point(size = 1.35) +
  geom_point(data = treffer, colour = "#b2182b", size = 2.3) +
  geom_text(
    data = treffer, aes(label = label),
    colour = "#b2182b", vjust = -0.9, size = 3.2
  ) +
  scale_colour_manual(values = c("#34495e", "#7f8c8d"), guide = "none") +
  coord_cartesian(ylim = c(0, 1.05)) +
  labs(
    x = "Position entlang der Chromosomen 1-3",
    y = expression(F[ST]),
    title = "Differenzierung Meer gegen Suesswasser"
  ) +
  theme_slide()
```
...
:::
::: {.column width="80%"}
**A** $F_{ST}$ aus $H_T$ und $H_S$  

**B** $F_{ST}$ vektorwertig aus der Trajektorie  
